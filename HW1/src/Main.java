public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < participants.length; i++) {
            System.out.println(participants[i].getClass().getSimpleName() + " начинает соревнование:");
            for (int j = 0; j < obstacles.length; j++) {
                if(obstacles[j].getMaxHeight() > participants[i].getMaxJump() || obstacles[j].getMaxLenght() > participants[i].getMaxRun()) {
                    ...
                    System.out.println(participants[i].getClass().getSimpleName() + " прошел " + obstacles[j].getClass().getSimpleName() + " и переходит к следующему испытанию");
                }
            }
            System.out.println("----------------------------------");
        }
